 ...
 * data set.
 */
Dygraph.prototype.xMinMax = function() {
  var min = this.rawData_[0][0];
  var max = this.rawData_[this.rawData_.length-1][0];
  var curr;
  for (var i=0; i < this.rawData_.length; ++i) {
    curr = this.rawData_[i][0];
    if (curr < min) {
      min = curr;
    }
    if (curr > max) {
...
Dygraph.prototype.xAxisExtremes = function() {
  ...
  var left = this.xMinMax().min;
  // var right = this.rawData_[this.rawData_.length - 1][0];
  var right = this.xMinMax().max;
  if (pad) {
